...
export default function Home() {
  ...
  useEffect(() => {
    ...
    atch(() => setLoading(false));
    }
  }, [session]);

  if (status === "loading") return <div className="flex items-center justify-center min-h-screen text-gray-500">로딩 중...</div>;
  ...
  const filtered = filter === "전체" ? emails.filter(e => e.status !== "기타") : emails.filter(e => e.status === filter);

  const counts: Record<string, number> = iv className="min-h-screen bg-gray-50">
      <header className="bg-white border-b px-6 py-4 flex justify-between items-center">
        <h1 className="text-xl font-bold text-gray-800">구직 활동 트래커</h1>
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-500">{session.user?.email}</span>
          <button onClick={() => signOut()} className="text-sm text-gray-400 hover:text-gray-600">로그아웃</button>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {statuses.map(s => (
            key={s}
              onClick={() => setFilter(s)}
              className={`px-4 py-1.5 rounded-full text-sm transition ${filter === s ? "bg-black text-white" : "bg-white text-gray-600 hover:bg-gray-100"}`}
            >
              {s}
            </button>
          ))}
              ...
              <div key={email.id} className="bg-white rounded-xl p-5 shadow-sm hover:shadow-md transition">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-400 mt-1 <p className="text-sm text-gray-500 mt-2 line-clamp-2">{email.snippet}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_COLORS[email.status]}`}>
                      {email.status}
                    </span>
                    <span className="text-xs text-gray-400">{new Date(email.date).toLocaleDateString("ko-KR")}</span>
                  </div>
                </div>
              </div>
            ))}
